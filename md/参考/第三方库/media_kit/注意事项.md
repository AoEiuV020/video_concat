...
社区讨论里已经有人直接追问 Linux 为什么不能像其他平台那样把 `libmpv` 一起打进产物。在 [issue #1055 的维护者回复](https://github.com/media-kit/media-kit/issues/1055#issuecomment-2525084942) 中，给出的结论很直接：**Linux 平台当前依赖 external libmpv**；如果要改成随应用分发，就需要自行处理 `libmpv` 及其依赖链。

这条依赖链并不轻。后续讨论提到，真要打包进 Flatpak / 应用目录，往往还要连同 ffmpeg、libplacebo 等组件一起处理，否则会继续卡在运行时依赖解析上。因此 Linux 目前更适合视为“系统库优先”平台，而不是默认的“预编译二进制全捆绑”平台。
