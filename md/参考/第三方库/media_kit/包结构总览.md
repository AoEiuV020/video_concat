...
> Linux 的视频和音频共用同一个包 `media_kit_libs_linux`，但 **Linux 构建阶段仍需系统安装 `mpv` 与 `libmpv-dev`**，供 CMake / pkg-config 解析 `PkgConfig::mpv`。

大多数平台实现包会通过各自构建系统自动集成原生依赖；Linux 额外依赖系统层的 `mpv` 开发包，不能只看 Dart 依赖是否齐全。
