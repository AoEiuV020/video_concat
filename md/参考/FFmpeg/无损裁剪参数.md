...
> 以下命令基于 **ffprobe 7.1.1**。旧版 `pkt_pts_time` 字段已废弃（[Trac #9543](https://trac.ffmpeg.org/ticket/9543)），使用 `pts_time` 替代。
...
| `-show_entries frame=pts_time` | 只输出 [PTS 时间戳](https://ffmpeg.org/ffprobe.html#show_005fentries)字段，隐式启用帧级输出（无需 `-show_frames`） |
| `-of default=noprint_wrappers=1:nokey=1` | 纯值输出，每行一个时间戳，[无节名包裹和字段名](https://ffmpeg.org/ffprobe.html#default) |
