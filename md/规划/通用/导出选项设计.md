...
```mermaid
flowchart TD
    HP[HomePage] --> VL[视频列表<br/>Expanded]
    HP --> BC[底部配置区]
    BC --> OCB[OutputConfigBar<br/>文件名 + 扩展名 + 导出选项开关]
    BC --> EOP[ExportOptionsPanel<br/>AnimatedSize 展开/收起]
    BC --> AB[操作按钮栏]
...
```mermaid
flowchart LR
    COL[Column] --> VL["视频列表<br/>Expanded<br/>min: 100px"]
    COL --> BC["底部配置区<br/>固定高度"]
    BC -->|展开时增高约 120px| EOP[ExportOptionsPanel]
```

- 视频列表为 `Expanded`，自适应剩余空间
- 底部配置区展开时视频列表自动缩短让出空间
- 展开/收起使用 `AnimatedSize` 平滑过渡
